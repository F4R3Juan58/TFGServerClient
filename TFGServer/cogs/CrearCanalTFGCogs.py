...
class CrearCanalTFGCogs(commands.Cog):
    ...
    async def crear_canal_tfg(self, insti_id: int, discord_id_profesor: int, discord_id_alumno: int):
        # Depuración: Verificación de entrada
        print(f"Debug: Creando canal TFG para instituto {insti_id}, profesor {discord_id_profesor}, alumno {discord_id_alumno}")
        ...
        # Nombre del canal será "TFG - Nombre del Alumno"
        nombre_canal = f"TFG - {alumno.display_name}"
        ...
        try:
            await canal.send(f"¡TFG iniciada en el canal {nombre_canal} para {alumno.display_name}!")
            print(f"Debug: Mensaje enviado al canal de texto.")
        except Exception as e:
            print(f"Error enviando mensaje de anuncio: {e}")

        return f"Canal TFG '{nombre_canal}' creado correctamente."

def setup(bot):
    bot.add_cog(CrearCanalTFGCogs(bot))